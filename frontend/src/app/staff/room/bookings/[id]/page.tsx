    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-6 flex gap-3">
        {booking.status === 'confirmed' && (
          <button onClick={() => updateStatus('checked_in')} className="rounded bg-zinc-800 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-700">
            Check in
          </button>
        )}
  ...
  return (
    <div className="rounded border border-zinc-200 p-4">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-zinc-400">{'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}</span>
      </div>
      ...
      {done && <p className="mt-2 text-xs text-zinc-600">Reply submitted.</p>}
    </div>
  );
}
