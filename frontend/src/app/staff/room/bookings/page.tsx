...
export default function StaffRoomBookingsPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const result = await apiClient.get<PaginatedResponse>(`/staff/room-bookings?${params}`);
  ...
  if (!user) {
    return <div className="mx-auto max-w-5xl px-4 py-16 text-center">
      <Link href="/login?redirect=/staff/room/bookings" className="rounded bg-zinc-900 px-4 py-2 text-sm font-medium text-white">Sign in</Link>
    </div>;
  }
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id} className="border-b border-zinc-100">
                    ...
                    <td className="py-3 pr-4">{new Date(b.checkIn).toLocaleDateString()}</td>
                    <td className="py-3 pr-4">{new Date(b.checkOut).toLocaleDateString()}</td>
                    ...
                    <td className="py-3">
                      <Link href={`/staff/room/bookings/${b.id}`} className="text-zinc-600 hover:text-zinc-900 text-xs">View</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
