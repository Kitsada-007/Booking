...
const createBoatSchema = z.object({
  ...
  date: z.string().min(1),
  boatCount: z.number().int().positive(),
  guestCount: z.number().int().positive(),
  paymentMethod: z.enum(['gateway', 'bank_transfer']),
});
...
router.post(
  ...
  async (req, res) => {
    try {
      const result = await createBoatBooking(req.user!.userId, req.body);
      res.status(201).json(result);
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Booking failed';
      if (msg === 'Boat type not found' || msg === 'Time slot not found') { res.status(404).json({ error: msg, code: 'NOT_FOUND' }); return; }
      if (msg === 'Not enough capacity available' || msg === 'Invalid date') { res.status(400).json({ error: msg, code: 'VALIDATION_ERROR' }); return; }
      res.status(500).json({ error: 'Failed to create booking', code: 'INTERNAL_ERROR' });
    }
  }
);

router.get('/boats', requireAuth, requireRole('member'), async (req, res) => {
  try {
    const result = await listMyBoatBookings(req.user!.userId, {
      status: typeof req.query.status === 'string' ? req.query.status : undefined,
      page: Number(req.query.page) || undefined,
      pageSize: Number(req.query.pageSize) || undefined,
    });
...
router.get('/boats/:id', requireAuth, requireRole('member'), async (req, res) => {
  try {
    const booking = await getMyBoatBooking(req.user!.userId, req.params.id as string);
    res.json(booking);
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Not found';
    if (msg === 'Booking not found') { res.status(404).json({ error: msg, code: 'NOT_FOUND' }); return; }
...
router.patch('/boats/:id/cancel', requireAuth, requireRole('member'), async (req, res) => {
  try {
    const booking = await cancelMyBoatBooking(req.user!.userId, req.params.id as string);
    res.json(booking);
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Cancel failed';
    if (msg === 'Booking not found') { res.status(404).json({ error: msg, code: 'NOT_FOUND' }); return; }
...
router.patch('/boats/:id/complete', requireAuth, requireRole('admin', 'boat_staff'), async (req, res) => {
  try {
    const booking = await completeBoatBooking(req.params.id as string);
    res.json(booking);
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Complete failed';
    if (msg === 'Booking not found') { res.status(404).json({ error: msg, code: 'NOT_FOUND' }); return; }
